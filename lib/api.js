...
const RefUtils = rquire('./utils')

class JsonRefs {
  constructor(options = {}, name) {
    this.name = name || 'JsonRefs'
    this.options = options
    var customRefUtils = options.refUtils || options.createRefUtils ? options.createRefUtils(options) : null
    this.utils = customRefUtils || new RefUtils(options)
  }
  ...
  // protected
  buildRefModel(document, options, metadata) {
    var allTasks = Promise.resolve();
    var subDocPtr = this.pathToPtr(options.subDocPath);
    var absLocation = this.utils.makeAbsolute(options.location, options);
    ...
    if (_.isUndefined(metadata.deps[docDepKey])) {
      ...
      refs = this.findRefs(document, options);
      var ctx = this;
      // Iterate over the references and process
      _.each(refs, (refDetails, refPtr) => {
        var refKey = makeAbsolute(options.location) + refPtr;
        var remoteLocation = this.utils.isRemote(refDetails) ? this.utils.combineURIs(relativeBase, refDetails.uri) : options.location;
        var anchor = '#' + (refDetails.uri.indexOf('#') > -1 ? refDetails.uri.split('#')[1] : '');

        var refdKey = refDetails.refdId = this.utils.makeAbsolute(remoteLocation) + anchor;
        ...
        if (!isValid(refDetails)) {
          this.log('invalid Ref', refDetails)
          return;
        }
        ...
        rOptions.subDocPath = _.isUndefined(refDetails.uriDetails.fragment) ? [] :
          this.pathFromPtr(this.utils.decodeURI(refDetails.uriDetails.fragment));

        // Resolve the reference
        if (this.utils.isRemote(refDetails)) {
          this.log('is remote', refDetails)
          ...
          allTasks = allTasks
            .then(function (nMetadata, nOptions) {
              return function () {
                var rAbsLocation = ctx.utils.makeAbsolute(nOptions.location, options);
                var rDoc = nMetadata.docs[rAbsLocation];

                if (_.isUndefined(rDoc)) {
                  // We have no cache so we must retrieve the document
                  return ctx.utils.getRemoteDocument(rAbsLocation, nOptions)
                    .catch(function (err) {
          ...
          this.log('not remote', refDetails)
          allTasks = allTasks
            .then(function () {
              return document;
            });
        ...
        allTasks = allTasks
          .then(function (nMetadata, nOptions, nRefDetails) {
            ctx.log('process remote', {
              nRefDetails
            })

            return function (doc) {
              if (_.isError(doc)) {
                ctx.utils.markMissing(nRefDetails, doc);
              } else {
                // Wrapped in a try/catch since findRefs throws
                try {
                  return ctx.buildRefModel(doc, nOptions, nMetadata)
                    .catch(function (err) {
                      ctx.utils.markMissing(nRefDetails, err);
                    });
                } catch (err) {
                  ctx.utils.markMissing(nRefDetails, err);
                }
  ...
  validateOptions(options, obj) {
    ...
    options.location = this.combineURIs(options.location, undefined);

    // Set the subDocPath to avoid everyone else having to compute it
    options.subDocPath = this.makeSubDocPath(options);
  ...
  decodePath(path) {
    ...
    return path.map(function (seg) {
      ...
      return this.utils.decodeURI(seg.replace(/~1/g, '/').replace(/~0/g, '~'));
    });
  }
  ...
  findRefs(obj, options) {
    ...
    // Validate options
    options = this.validateOptions(options, obj);

    // Walk the document (or sub document) and find all JSON References
    this.utils.walk(this.utils.findAncestors(obj, options.subDocPath),
      this.utils.findValue(obj, options.subDocPath),
      ...
      function (ancestors, node, path) {
        ...
        if (this.utils.isRefLike(node)) {
          ...
          refDetails = this.getRefDetails(node);
          ...
          if (this.utils.getExtraRefKeys(node).length > 0) {
  ...
  findRefsAt(location, options) {
    ...
    allTasks = allTasks
      .then(function () {
        ...
        return this.utils.getRemoteDocument(options.location, options);
      })
      .then(function (res) {
        var cacheEntry = _.cloneDeep(this.remoteCache[options.location]);
        var cOptions = _.cloneDeep(options);
        var uriDetails = this.utils.parseURI(options.location);
        ...
        if (_.isUndefined(cacheEntry.refs)) {
          ...
          this.remoteCache[options.location].refs = this.findRefs(res, cOptions);
        }
        ...
        if (!_.isUndefined(uriDetails.fragment)) {
          cOptions.subDocPath = this.pathFromPtr(decodeURI(uriDetails.fragment));
        ...
        return {
          refs: this.findRefs(res, cOptions),
          value: res
        };
      });
  ...
  getRefDetails(obj) {
    ...
    try {
      if (this.utils.isRefLike(obj, true)) {
        cacheKey = obj.$ref;
        uriDetails = uriDetailsCache[cacheKey];

        if (_.isUndefined(uriDetails)) {
          uriDetails = uriDetailsCache[cacheKey] = this.utils.parseURI(cacheKey);
        }
        ...
        if (_.isUndefined(uriDetails.error)) {
          details.type = this.utils.getRefType(details);
        } else {
        ...
        // Identify warning
        extraKeys = this.utils.getExtraRefKeys(obj);
   ...
   * }
   */
  isRef(obj, throwWithDetails) {
    return this.utils.isRefLike(obj, throwWithDetails) && this.getRefDetails(obj, throwWithDetails).type !== 'invalid';
  }
  ...
  pathFromPtr(ptr) {
    ...
    return this.decodePath(segments);
  }
   ...
   */
  pathToPtr(path, hashPrefix) {
    if (!_.isArray(path)) {
      throw new Error('path must be an Array');
    }

    // Encode each segment and return
    return (hashPrefix !== false ? '#' : '') + (path.length > 0 ? '/' : '') + this.encodePath(path).join('/');
  }
  ...
  resolveRefs(obj, options) {
    ...
    allTasks = allTasks
      ...
      .then(function () {
        ...
        return this.buildRefModel(obj, options, metadata)
          .then(function () {
            return metadata;
          });
      })
      .then(function (results) {
        ...
        var fullLocation = this.utils.makeAbsolute(options.location);
        var refsRoot = fullLocation + this.pathToPtr(options.subDocPath);
        ...
        _.each(results.deps, function (props, node) {
          _.each(props, function (dep, prop) {
            ...
            var remote = this.utils.isRemote(refDetails);
            var pathIndex;
        ...
        _.each(results.deps, function (deps, parentPtr) {
          ...
          var pPtrPath = this.pathFromPtr(pPtrParts[1]);

          _.each(deps, function (dep, prop) {
            var depParts = dep.split('#');
            var dDocument = results.docs[depParts[0]];
            var dPtrPath = pPtrPath.concat(this.pathFromPtr(prop));
            var refDetails = results.refs[pPtrParts[0] + this.pathToPtr(dPtrPath)];
            ...
            if (_.isUndefined(refDetails.error) && _.isUndefined(refDetails.missing)) {
              if (!options.resolveCirculars && refDetails.circular) {
                ...
                try {
                  refDetails.value = this.utils.findValue(dDocument, this.pathFromPtr(depParts[1]));
                } catch (err) {
                  this.utils.markMissing(refDetails, err);
                ...
                if (pPtrParts[1] === '' && prop === '#') {
                  ...
                  this.utils.setValue(pDocument, dPtrPath, refDetails.value);
                }
              }
            }
          });
        ...
        walkRefs(root, refPtr, refPath) {
          ...
          allRefs[refPtrParts[0] === options.location ?
            '#' + refPtrParts[1] :
            this.pathToPtr(options.subDocPath.concat(refPath))] = refDetails;
          ...
          if (refDetails.refdId.indexOf(root) !== 0) {
            Object.keys(refDeps).forEach(function (prop) {
              this.walkRefs(refDetails.refdId, refDetails.refdId + prop.substr(1), refPath.concat(this.pathFromPtr(prop)));
            });
          }
        }
        ...
        Object.keys(results.refs).forEach(function (refPtr) {
          ...
          walkRefs(refsRoot, refPtr, this.pathFromPtr(refPtr.substr(refsRoot.length)));
        });
   ...
   *   });
   */
  resolveRefsAt(location, options) {
    var allTasks = Promise.resolve();
    ...
    allTasks = allTasks
      ...
      .then(function (res) {
        ...
        if (!_.isUndefined(uriDetails.fragment)) {
          cOptions.subDocPath = this.pathFromPtr(this.utils.decodeURI(uriDetails.fragment));
        }

        return this.resolveRefs(res, cOptions)
          .then(function (res2) {
            return {
              refs: res2.refs,
...
const api = new JsonRefs();

/* Export the module members */
module.exports.clearCache = api.clearCache;
module.exports.decodePath = api.decodePath;
module.exports.encodePath = api.encodePath;
module.exports.findRefs = api.findRefs;
module.exports.findRefsAt = api.findRefsAt;
module.exports.getRefDetails = api.getRefDetails;
module.exports.isPtr = api.isPtr;
module.exports.isRef = api.isRef;
module.exports.pathFromPtr = api.pathFromPtr;
module.exports.pathToPtr = api.pathToPtr;
module.exports.resolveRefs = api.resolveRefs;
module.exports.resolveRefsAt = api.resolveRefsAt;
module.exports.JsonRefs = JsonRefs;
