  ...
  buildRefModel(document, options, metadata) {
    ...
    if (_.isUndefined(metadata.deps[docDepKey])) {
      ...
      _.each(refs, (refDetails, refPtr) => {
        ...
        var remoteLocation = this.utils.isRemote(refDetails, options) ? this.utils.combineURIs(relativeBase, refDetails.uri) : options.location;
        var anchor = '#' + (refDetails.uri.indexOf('#') > -1 ? refDetails.uri.split('#')[1] : '');

        var refdKey = refDetails.refdId = this.utils.makeAbsolute(remoteLocation, options) + anchor;
        ...
        if (this.utils.isRemote(refDetails, options)) {
  ...
  resolveRefs(obj, options) {
    ...
    allTasks = allTasks
      ...
      .then(function (results) {
        ...
        var fullLocation = this.utils.makeAbsolute(options.location, options);
        ...
        _.each(results.deps, function (props, node) {
          _.each(props, function (dep, prop) {
            ...
            var remote = this.utils.isRemote(refDetails, options);
            var pathIndex;
