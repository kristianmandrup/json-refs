...
class JsonRefs {
  ...
  buildRefModel(document, options, metadata) {
    ...
    if (_.isUndefined(metadata.deps[docDepKey])) {
      ...
      _.each(refs, (refDetails, refPtr) => {
        var refKey = this.utils.makeAbsolute(options.location, options) + refPtr;
        ...
        rOptions.subDocPath = _.isUndefined(refDetails.uriDetails.fragment) ? [] :
          this.pathFromPtr(decodeURI(refDetails.uriDetails.fragment));
  ...
  validateOptions(options, obj) {
    ...
    options.location = this.utils.combineURIs(options.location, undefined);
    ...
    this.utils.walk(this.utils.findAncestors(obj, options.subDocPath),
      ...
      function (ancestors, node, path) {
        ...
        if (self.utils.isRefLike(node)) {
          ...
          refDetails = self.getRefDetails(node);
          ...
          if (self.utils.getExtraRefKeys(node).length > 0) {
